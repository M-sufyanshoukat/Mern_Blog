...
export const signup = (first_name, last_name, email, password) => async (dispatch) => {
	...
	try {
		...
		const { data } = await axios.post('api/users/signup', { first_name, last_name, email, password }, config);
...
export const sendResetEmail = (email) => async (dispatch) => {
	...
	try {
		...
		const { data, status } = await axios.post(`/api/auth/reset-password-request`, { email }, config);
